    ...
    @if (isset($category->category_name) && $category->category_name)
        <div class="back-button-container">
            <div class="row">
                <div class="col-sm-12">
                    @if ($category->parent)
                        <a href="{{ route(LocaleHelper::localePrefix() . 'vendirun.category', [urlencode($category->parent->category_name)]) }}" class="btn btn-default"><i class="fa fa-angle-double-left"></i> {{ trans('vendirun::property.backTo') }} {{ $category->parent->category_name }}
                        </a>
                    @else
                        <a href="{{ route(LocaleHelper::localePrefix() . 'vendirun.category', ['']) }}" class="btn btn-default"><i class="fa fa-angle-double-left"></i> {{ trans('vendirun::property.backToCategories') }}</a>
                    @endif
                </div>
            </div>
        </div>
        <div class="row">
            ...
            <div class="col-sm-6">
                ...
                <a class="btn btn-default" href="{{ route(LocaleHelper::localePrefix() . 'vendirun.propertySearch') }}/?propertytype={{ $category->id }}">{{ trans('vendirun::property.viewProperties') }}</a>
            </div>
        </div>
        <hr>
    @endif
                ...
                <div class="col-sm-6 col-md-{{ $element_options['col_md'] }}">
                    <div class="thumbnail">
                        @if ($element_options['show_images'] == 'Yes')
                            <a href="{{ route(LocaleHelper::localePrefix() . 'vendirun.category', [urlencode($subCategory->link_name)]) }}">
                        ...
                        <div class="buttons">
                            <a class="btn btn-default" href="{{ route(LocaleHelper::localePrefix() . 'vendirun.propertySearch') }}/?propertytype={{ $subCategory->id }}">{{ trans('vendirun::property.viewProperties') }}</a>
                            <a class="btn btn-primary" href="{{ route(LocaleHelper::localePrefix() . 'vendirun.category', [urlencode($subCategory->link_name)]) }}">{{ trans('vendirun::property.viewCategory') }}</a>
                        </div>
                    </div>
                </div>
            @endforeach
